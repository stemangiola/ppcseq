...
saveRDS(input_2, "dev/input_2.rds", compress = "gzip")
		...
		`inference` =
			map2(fp, `data source`, ~
					 	...
					 	identify_outliers(
					 		...
					 		approximate_posterior_inference = F,
					 		approximate_posterior_analysis = F,
					 		do_check_only_on_detrimental = F
					 	)
			)
	)
...
es_to_stats = function(es){
	es %>%
		...
		mutate(P = map_int(
			data,
			~ .x %>%
				count(outlier_detected = `ppc samples failed` > 0) %>%
				filter(outlier_detected ) %>%
				pull(n) %>% when(length(.)==0 ~ 0L, ~ (.))
		)
		) %>%

		mutate(FP_over_P = FP_rate / P)
}
...
# p1 =
# 	stats %>%
# 	ggplot(aes(x = fp/100, y = FP_over_P )) +
